---
title: MaskedDiv
description: A React component that applies SVG mask shapes to images or videos, supporting multiple mask types and responsive sizing.
---

import { PreviewContainer } from "@/components/PreviewContainer";
import { MaskeddivDemo } from "@/components/lyx/maskeddiv-demo";
...
```bash
npx shadcn@latest add http://lyxui.wisplabs.xyz/r/masked-div.json
```
...
```bash
yarn shadcn@latest add http://lyxui.wisplabs.xyz/r/masked-div.json
```
...
```bash
pnpm dlx shadcn@latest add http://lyxui.wisplabs.xyz/r/masked-div.json
```
...
```bash
bunx --bun  add http://lyxui.wisplabs.xyz/r/masked-div.json
```
...
<Steps>
<Step>
Copy the source code to `components/lyxui/masked-div.tsx`
```js
"use client";
...
<TypeTable
  type={{
    children: {
      description: "A single image or video React element to be masked.",
      type: "React.ReactElement<HTMLImageElement | HTMLVideoElement>",
      default: "required",
    },
    maskType: {
      description: "The SVG mask shape to use.",
    ...
    backgroundColor: {
      description: "Background color behind the masked content.",
      type: "string",
      default: '"transparent"',
    },
    size: {
      description:
        "Relative width of the masked container (0-1, as a fraction of parent width).",
      type: "number",
      default: "1",
    },
  }}
